...
var headpic = {
    init: function() {
        //发布的时候需要删除该调试文件 --- BEGIN
        // loadscript("http://10.9.248.120:8080/target/target-script-min.js#anonymous");
        //发布的时候需要删除该调试文件 --- END
        var me = this;
        me.setCanvasXY();
        me.bindEvent();
    ...
    handleUpload: function(file, callback) {
        ...
        imageReader.onload = function() {
            var el = document.getElementById("canvas_img");
            //图片加载完成之后再执行回调函数
            el.onload = function() {
...
function fixUploadImgBug() {
    $(".file").off("change").remove();
    $("#upload").prepend('<input class="file" type="file" name="imageurl" accept="image/*">');
    $(".file").on("change", fileChange);
}
